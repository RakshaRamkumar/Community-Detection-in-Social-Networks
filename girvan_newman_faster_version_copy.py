#Source of the original code: https://github.com/mansiganatra/Girvan-Newman-Implementation-using-Spark/blob/master/Mansi_Ganatra_Task2.py
...
if len(sys.argv) != 5:
    print("Usage: spark-submit	girvan-newman.py  <filter_threshold> <input_file_path> <betweenness_output_file_path> <community_output_file_path>")
    exit(-1)
else:
...
result = []
SparkContext.setSystemProperty("spark.memory.offHeap.enabled","true")
SparkContext.setSystemProperty("spark.memory.offHeap.size","100g")
SparkContext.setSystemProperty('spark.executor.memory', '100g')
SparkContext.setSystemProperty('spark.driver.memory', '100g')
sc = SparkContext('local[*]', 'GirvanNewman')
ss = SparkSession(sc)

N_values = [2, 4, 8, 16, 32, 64, 72]
exec_times = []
modularities = []
...
for N in N_values:
    ...
    finalRdd = user_businessRdd.filter(lambda entry: entry[0] != headers[0][0]).repartition(N).cache()
...
plt.savefig('plot_girvan_newman.png', bbox_inches = 'tight')